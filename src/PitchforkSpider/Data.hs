...
module PitchforkSpider.Data (runTests) where
...
parseRatingFromTombstoneAlbum = withObject "Rating" $ \o ->
  do
    mbRating <- o .:? "rating"
    return $ do
      ratingStr <- mbRating
...
testReviewParser = hspec $ do
    ...
    it "should work in the happy case" $ do
      review
        `shouldBe`
          ...
          , url = "https://pitchfork.com/reviews/albums/the-front-bottoms-in-sickness-and-in-flames/"
  ...
  where
    ...
    fixture = object
        ...
        , "url" .= String "/reviews/albums/the-front-bottoms-in-sickness-and-in-flames/"
...
testParseRatingFromTombstoneAlbum = hspec $ do
  describe "parseRatingFromTombstoneAlbum" $ do
    it "should extract return the value if it is a valid numeric string" $ do
      let result = parseEither parseRatingFromTombstoneAlbum fixtureWithValidRating
      result `shouldBe` Right (Just 5.4)

    it "should extract return Nothing if the rating is invalid" $ do
      let result = parseEither parseRatingFromTombstoneAlbum fixtureWithBadRating
      result `shouldBe` Right Nothing
      
    it "should extract return Nothing if the rating key is missing" $ do
      let result = parseEither parseRatingFromTombstoneAlbum fixtureWithoutRating
      result `shouldBe` Right Nothing
  where
    fixtureWithValidRating :: Value
    fixtureWithValidRating =
      object
        [ "rating" .= String "5.4" ]

    fixtureWithBadRating :: Value
    fixtureWithBadRating =
      object
        [ "rating" .= String "not a number" ]

    fixtureWithoutRating :: Value
    fixtureWithoutRating =
      object [ ]
